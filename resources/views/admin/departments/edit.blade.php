                ...
                <form action="{{ route('admin.departments.update', $department) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="group" name="group"
                                ...
                                onblur="this.style.boxShadow='none'; this.style.transform='none';"
                                required>
                            <option value="manufacturing" {{ old('group', $department->group) == 'manufacturing' ? 'selected' : '' }}>Manufacturing (Produksi)</option>
                            <option value="office" {{ old('group', $department->group) == 'office' ? 'selected' : '' }}>Office</option>
                        </select>
                        @error('group')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
